...
const outDir = path.resolve('internal-release/second-bundle');
fs.mkdirSync(outDir, { recursive: true });

const config = {
  projectName: 'Internal Release Demo Case 02 - Pass',
  panelType: 'external',
...
const verification = {
  ...
  exportedAt,
  passCaseSummary: {
    rationale: 'Second internal bundle uses a real pass case tuned from the first bundle by increasing declared screw resistance and widening support widths while keeping the same overall project-mode export flow.',
    changedInputs: {
      screwStrength: config.screwStrength,
      supportWidths: config.supportWidths,
    },
  },
  snapshot: {
    path: snapshotPath,
  ...
  result: {
    ...
    utilizationRatios: resultPkg.resultSnapshot.utilizationRatios,
